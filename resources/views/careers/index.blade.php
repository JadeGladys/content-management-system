...
@section('content')
            ...
            <div class="mb-8 rounded-xl border border-slate-200 bg-gradient-to-br from-white via-slate-50 to-blue-50/70 px-4 py-4 shadow-sm md:px-6">
                <div class="grid gap-4 lg:grid-cols-[minmax(0,1fr)_15rem] lg:items-center">
                    <div class="max-w-4xl">
                        ...
                        <h1 class="mt-3 text-3xl leading-tight font-semibold tracking-tight text-slate-900 md:text-[2.35rem]">Manage every CMS career from one view</h1>
                    ...
                    <div class="rounded-2xl border border-white/80 bg-white/80 px-4 py-3 shadow-sm backdrop-blur">
                        ...
                        <p class="mt-1 text-xl font-semibold text-slate-900">{{ $careers->total() }}</p>
                    ...
                    <table class="min-w-[950px] w-full table-fixed">
                        ...
                        <thead class="bg-slate-50 text-left text-[13px] font-semibold text-slate-900">
                            <tr>
                                ...
                                <th scope="col" class="px-3 py-4">Career title</th>
                                <th scope="col" class="px-3 py-4">Category</th>
                                <th scope="col" class="px-3 py-4">Type</th>
                                <th scope="col" class="px-3 py-4">Created By</th>
                                ...
                                <tr class="transition hover:bg-slate-50/80 has-[:checked]:bg-blue-50/50">
                                    ...
                                    <td class="px-4 py-5 font-medium text-slate-900">
                                        <div class="min-w-0 max-w-[16rem] xl:max-w-[18rem]">
                                            <span class="block truncate text-sm font-semibold leading-6" title="{{ $listedCareer->title }}">
                                                {{ $listedCareer->title }}
                                            </span>
                                        </div>
                                    </td>
                                    ...
                                    <td class="px-4 py-5 text-slate-500">
                                        <span class="block truncate" title="{{ $listedCareer->type }}">
                                            {{ $listedCareer->type ? Str::of($listedCareer->type)->replace('_', ' ')->title() : '—' }}
                                        </span>
                                    </td>
            ...
            <div class="mx-auto mt-4 flex flex-wrap items-center justify-between gap-4">
                ...
                @if ($careers->hasPages())
                    <nav aria-label="Pagination" class="flex w-max items-center overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm divide-x divide-slate-200">
                        ...
                        @else
                            <a href="{{ $careers->previousPageUrl() }}" class="flex h-11 w-11 shrink-0 items-center justify-center hover:bg-slate-50 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500">
                                ‹
                            </a>
                        @endif
                        ...
                        @foreach (range(1, $careers->lastPage()) as $page)
                            @if ($page >= max(1, $careers->currentPage() - 2) && $page <= min($careers->lastPage(), $careers->currentPage() + 2))
                                ...
                                @else
                                    <a href="{{ $careers->url($page) }}" class="flex h-11 w-11 shrink-0 items-center justify-center text-sm font-semibold text-slate-900 hover:bg-slate-50 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500">
                                        {{ $page }}
                                    </a>
                                @endif
                            @endif
                        ...
                        @if ($careers->hasMorePages())
                            <a href="{{ $careers->nextPageUrl() }}" class="flex h-11 w-11 shrink-0 items-center justify-center hover:bg-slate-50 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500">
                                ›
                            </a>
                        @else
                            <span class="flex h-11 w-11 shrink-0 items-center justify-center text-slate-300">
    ...
    <x-filter-panel
        panel-id="careerFilterPanel"
        open-button-id="openCareerFilterPanel"
